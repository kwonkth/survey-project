...
function buildUserPrompt({ topic, questionCount, style, styleTemplate, includeNameQuestion, questionTypeMode, mandatoryQuestions }) {
  ...
  return [
    ...
    '- mode = fixed_five  : 모든 객관식 문항은 5지선다 (radio)로 생성',
    ...
    '',
    '질문/보기 의미 일치 규칙:',
    '- 각 questions[i].text는 사람이 읽을 수 있는 자연어 질문 문장이어야 하며, "문항 1-4지선다", "1번 보기", "A. 예 B. 아니오" 같은 메타 제목이나 번호를 넣지 않는다.',
    '- 먼저 문항이 묻고자 하는 의미를 정한 뒤, 그 의미에 맞는 보기(options)를 설계한다.',
    '- 또는 이미 정해진 보기 후보가 있다면, 그 보기들이 무엇을 의미하는지 자연스럽게 설명하는 질문 문장을 text 필드에 작성한다.',
    '',
    '잘못된 예시:',
    'text: "문항 1-4지선다"',
    'type: "radio"',
    ...
    '',
    '올바른 예시:',
    '{ "text": "지금 이 선택이 당신에게 중요한 결정이라고 느껴지나요?", "type": "radio", "options": ["예", "아니오"] }',
    '',
    '또 다른 예시 (감정 선택):',
    'options: ["평화롭다", "혼란스럽다", "불안하다"]',
    '→ text: "현재 상황에서 가장 가까운 감정은 무엇인가요?"',
    '',
