  ...
  try {
    ...
    try {
      await env.surveyforge
        .prepare(`INSERT INTO surveys (survey_id, title, description, questions, story, created_at, updated_at)
                  VALUES (?, ?, ?, ?, ?, ?, ?)`)
        .bind(survey_id, title, description ?? null, questionsText, storyText, createdAt, updatedAt)
        .run();
    } catch (e) {
      if (String(e?.message || '').includes('UNIQUE')) {
        await env.surveyforge
          .prepare(`UPDATE surveys SET title = ?, description = ?, questions = ?, story = ?, updated_at = ? WHERE survey_id = ?`)
          .bind(title, description ?? null, questionsText, storyText, updatedAt, survey_id)
          .run();
      } else {
        return Response.json({ error: 'DB error', detail: String(e?.message || e) }, { status: 500 });
      }
